...
cluster_names_and_description = get_cluster_names_and_descriptions()

predict_cluster_id = predict_model(model, data=person_df)['Cluster'].values[0]
predicted_cluster_data = cluster_names_and_description[predict_cluster_id]

st.header(f"Najbliżej Ci do grupy: {predicted_cluster_data['name']}")
st.markdown(predicted_cluster_data['description'])
same_cluster_df = all_df[all_df['Cluster'] == predict_cluster_id]
